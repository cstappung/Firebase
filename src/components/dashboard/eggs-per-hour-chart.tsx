...
import { subscribeToEggsPerHour } from "@/lib/data";
...
type ChartData = Awaited<ReturnType<typeof subscribeToEggsPerHour>>;
...
export function EggsPerHourChart({ henhouse }: EggsPerHourChartProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToEggsPerHour(
      henhouse,
      startDate,
      endDate,
      startHour,
  ...
  const handleDownload = () => {
    const csvContent =
      ...
      "day,hour,eggs\n" +
      chartData.map((d: any) => d.data.map((e: any) => `${d.date},${e.hour},${e.eggs}`).join("\n")).join("\n");
  ...
  const flattenedData = chartData.flatMap((d: any) => d.data.map((h: any) => ({ name: `${d.date} ${h.hour}`, eggs: h.eggs })));
